...
ggplot(cw_tr, aes(x = as.numeric(intersect_area))) + geom_histogram() + scale_x_log10()
```
...
#### Number of Households
```{r hhtotal}
raw_hh <- get_acs("tract", variables = "B08202_001", state = "UT", county = county_list)

hh <- raw_hh %>%
  left_join(acsvars, by = c("variable" = "name")) %>%
  separate(label, c("VAR", "total", "households"), sep = "!!") %>%
  select(GEOID, estimate) %>% 
  group_by(GEOID) %>% 
  summarise(HHBASE = sum(estimate))
