...
#This section to add hh_id doesn't work yet, I manually added it in Excel for now
hh <- 0  
# TODO: replace this with a join on the HH_ID field, please!!
for (i in nrow(clean_per)){
  pernum <- clean_per[i,"SPORDER"]
  if (as.numeric(pernum) == 1){
    hh <- hh + 1
    clean_per$hh_id[i] = hh
  }
  else{clean_per$hh_id[i] = hh}
}
