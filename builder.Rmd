...
### Obtain geometries
We are going to build this synthetic population for Provo and Orem, Utah. This
involves the PUMAs `49002` and `49003`. We can get the census tracts these PUMAs
...
puma_tract <- read_csv(puma_url, col_types = list(PUMA5CE = col_character())) %>%
  transmute(
    TRACT = str_c(STATEFP, COUNTYFP, TRACTCE),  
    PUMA = PUMA5CE
  ) %>%
  filter(PUMA %in% c("49002", "49003"))
```
...
taz_control <- lapply(tcf[grepl("SE_", tcf)], function(csv) {
  read_csv(csv) %>% select(TAZ = TAZID, HHBASE = TOTHH) %>%
    mutate(HHBASE = ceiling(HHBASE))
}) %>%
  bind_rows() %>%
  filter(HHBASE > 0) %>%
...
Replace NA fields with a negative integer to avoid errors and write to disk.
```{r remove_na}
seed_hh %>% write_csv("data/seed_households.csv")

seed_per %>% write_csv("data/seed_persons.csv")
```
