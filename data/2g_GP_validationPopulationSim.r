...
#
# This script was build using following version of R
# -------------------------------------------
...
#
# JXR updated the script for GP output validation on 9/26/2019
#
##############################################################################################################
...
columnMap       <- read.csv(paste(validationDir, "/data/GP_columnMapPopSim_SOABM.csv", sep = "/"))

seed_households <- read.csv(paste(popSimDir, "/data/GP_seed_households.csv", sep = "/"))
...
summary2           <- read.csv(paste(popSimDir, "/output/summary_MAZ_PUMA.csv", sep = "/"))
summary3           <- read.csv(paste(popSimDir, "/output/summary_TAZ.csv", sep = "/"))      
summary4           <- read.csv(paste(popSimDir, "/output/summary_MAZ.csv", sep = "/"))
...
write.csv(stats, paste("output/", scenario, "GP_PopulationSim_stats.csv"), row.names = FALSE)
...
ggsave(file=paste("plots/", scenario, "GP_PopulationSim_Convergance_sdev.jpeg"), width=8,height=10)
...
ggsave(file=paste("plots/", scenario, "GP_PopulationSim_Convergance_PRMSE.jpeg"), width=8,height=10)
...
summary_hhid <- summary_hhid[, c("FINALWEIGHT", expanded_hhid_col)]

summary_hhid <-  summary_hhid %>%
  group_by(hh_id) %>%
...
ggsave("plots/GP_EF_Distribution.png", width=15,height=10)
...
write.csv(uAnalysisPUMA, "output/GP_uniformity.csv", row.names=FALSE)
