...
The coordinate system (CRS) that was used in the GIS calculation of the latitude and longitude points was WGS 84. Both the TAZ centroids and address points are in the same coordinate system.

```{r files}
#the synthetic population and address datasets are read into the file
#we only read in the residential type addresses
house <- read_csv("output/synthetic_households.csv") 
address <- read_csv("inputs/AddressCoordinates.csv") %>%
...
persons <- read_csv("output/synthetic_persons.csv")
personsf <- read_csv("output/final_persons.csv")
```
...
In order to accomplish the task of assigning households address point coordinates, first the total number of households in each TAZ must be determined.
...
#totp <- persons %>%
#  arrange(TAZ) %>% select(TAZ,household_id) 
#left_join(totp, address, by = "TAZ")
#paged_table(totp)
...
Then, by spliting up the address table into separate vectors for each TAZ, and using a loop, a set of coordinates for the right amount of households per TAZ can be determined.The separted vectors with added coordinates are then joined into a table.
...
house2 <- house %>%
  ...
  select(tazcom, index, household_id,PUMA,TRACT,NP,HHINCADJ,WIF)
...
The table below shows the final result of the household coordinates in longitude and latitude. It is a joint table of the updated address point coordinates and the house data table.

```{r}
#join the address point and house tables by the indexing of the TAZ, thus assigning coordinates to the households
hhcoord <- left_join(house2, address2, by = "tazcom") %>% select(TAZ.x, household_id,PUMA,TRACT,NP,HHINCADJ,WIF, longitude, latitude)
...
#### Updating ActivitySim Inputs and Outputs
The household coordinates table is just the coordinates added to the synthetic households csv from the ActivitySim inputs. We also want to add these coordinates to the other input of the sythetic persons as well as the final persons ActivitySim output. That way, household coordinates are in all the important tables that are needed.

```{r}
persons <- persons %>%
  arrange(TAZ) %>%
  left_join(persons,hhcoord, by = "TAZ") 
