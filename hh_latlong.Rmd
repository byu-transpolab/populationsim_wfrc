...
tazc <- read_csv("inputs/tazcentroids.csv") %>%
  arrange(TAZID) %>%
  filter(CO_FIPS > 0) %>%
...
In order to accomplish this, first the total number of households in each TAZ is determined.
```{r}
tothh <-house %>%
...
Then, by spliting up the parcel table into separte vectors for each TAZ, and using a loop, a set of coordinates for the right amount of households per TAZ can be determined.
```{r}
