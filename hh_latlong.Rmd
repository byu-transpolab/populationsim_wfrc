...
```{r}
#read in the gis file for the taz polygons and centroids
taz <- st_read("inputs/taz.geojson") %>% 
  ...
  filter(TAZ == tazid) %>%
  select(TAZ)
tazc <- taz %>% select(TAZ) %>% st_centroid()
```
...
Finally, a comparison map can be seen. The left showing the household coordinates before this analysis, and the right showing the position of the households after this analysis.The positioning of the new coordinates for the households are clearly in more reasonable spots.
