...
The acre data comes from the buildings, parcels, and TAZ shapefile/urbanization data from WFRC. The total number of acres comes from the urbanization file. Other than the total number of acres, the urbanization file only has a "Developed Acres" field, which is not specific enough to use for the residential and commercial acres fields. Therefore, calculations using the parcels and buildings data was used to get an estimate of the residential and commercial number of acres.
...
***
## Final Table
The final table is the combination of all 42 column headers (described above), organized primarily by the socioeconomic data from the 2018 SE WFRC file. This table will be used as one of the primary inputs for the ActivitySim model. 

Note: Some districts when computted were not found in the QGIS TAZ table and therefore their corresponding rows were removed in the final table. Most of these rows had the value of 0 for the rest of their column categories anyway so I assume this is okay to do. Also, since the data isn't perfect many cells displayed "NA". This is changed to the value of 0.
