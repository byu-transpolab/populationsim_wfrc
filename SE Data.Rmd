...
***
### DISTRICT, SD, COUNTY
The District and County information is determined using the data in the TAZ shapefile. The Districts are numbered from 1-17, and the Counties are numbered 1-4. County 1 is Weber, County 2 is Davis, County 3 is Salt Lake, and County 4 is Utah.  

```{r}
#use and select the disctric and county values found in the TAZ table
dc <- tazgis %>%
  mutate(
...
A table was made from joining the TAZ and Contour shapefiles, and the various eleveations for each range are displayed within the table. The table below shows the relationship between the difference in elevation in regards to its steepness.  
