...
***
### Data Sources
In order to put together all 42 columns of input data, the data must be collected from various sources. The data sources are as follows: the 2018 SE WFRC data files, the synthetic population files, the buildings, parcel, and urbanization datasets from WFRC, the TAZ shapefile data, and lastly the topology data, box elder parcel data, and school data all from the AGRC website joint with the TAZ shapefile.
...
#box elder parcel data from AGRC
parcels_be <- read_csv("inputs/other/parcels_BoxElder.csv")
  colnames(parcels_be)[1] <- "ZONE"
...
The residential and commercial acres for the Box Elder county was not included in the WFRC parcels dataset, so this data was determined from another source and by assumption. The RESACRE field was determined from a parcel dataset from AGRC joint with the WFRC TAZ shapefile. 

https://gis.utah.gov/data/cadastre/parcels/

The Box Elder RESACRE field is the total number of private acres per TAZ from the joint data. The Box Elder CIACRE field is just assumed to be 11% of the TOTACRE field (average of all the other CIACRE field values), since it isn't included in the joint data calculated.

```{r}
#calculate Box Elder residential acres
beres <- parcels_be %>%
  mutate(RESACRE = ifelse(OWN_TYPE == "Private", ACRES,0))%>%
  select(ZONE,RESACRE) %>% group_by(ZONE) %>% summarize_all("sum")

#calculate Box Elder commercial acres
becity <- totacre %>%
  filter(ZONE < 141) %>% mutate(CIACRE = .11 * TOTACRE) %>% 
  select(ZONE, CIACRE, TOTACRE)

#compute a table of the box elder acre data and all the zones' acre data
beacres <- left_join(beres,becity, by = "ZONE") %>% mutate(RESACRE = ifelse(TOTACRE == 0, 0, RESACRE)) %>% select(ZONE, RESACRE, CIACRE)
allacres <- rbind(beacres, acres)

#update full table
FullTable <- left_join(FullTable, allacres, by = 'ZONE', all = TRUE)
```
...
FullTable[,c(1:14,19:30,33,39:42)] <- sapply(FullTable[,c(1:14,19:30,33,39:42)], as.integer)
```
